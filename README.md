...
По умолчанию используется `https://classic.dzzzr.ru/moscow/`. Флаг `-city`
меняет город в пути; `-base-url` или `DZZZR_BASE_URL` задаёт адрес движка целиком.
...
dzzzr web                                    # тот же агент в браузере, рядом — редактор игры
```
...
Переменные окружения не обязательны: при первом запуске `dzzzr web` или
`dzzzr editor` открывается мастер настройки. Он проходит два шага: подключает
модель и выполняет вход в Дозор игроком или организатором. Способы подключения
модели:

- **Polza.ai** — вход в аккаунт или API-ключ, выбор модели с поддержкой
  инструментов, проверка баланса;
- **подписка ChatGPT** — вход через браузер; если уже выполнен вход в Codex CLI
  (`~/.codex/auth.json`), dzzzr использует его;
- **OpenAI-совместимый провайдер** — адрес API, ключ и модель (по умолчанию
  OpenRouter).

Редактору модель не нужна, поэтому её настройку можно отложить кнопкой
«Настроить позже». Позже настройки меняются кнопкой ⚙ рядом с названием модели,
а мастер открывается снова кнопкой ◈.

Настройки сохраняются в `<каталог настроек>/llm/settings.json`, вход ChatGPT —
в `codex/auth.json` рядом. Оба файла доступны только владельцу. Их читают и веб,
и `dzzzr chat`, и `dzzzr agent`. Выход из аккаунта игрока их не удаляет.
Переменные окружения (`DZZZR_LLM_PROVIDER`, `DZZZR_LLM_API_KEY`,
`DZZZR_LLM_BASE_URL`, `DZZZR_LLM_MODEL` и их варианты `LLM_*`, `OPENROUTER_*`)
важнее сохранённого. Панель показывает, какая переменная перекрывает значение.
...
Рядом с чатом — переключатель «Редактор» (`dzzzr editor` открывает браузер
сразу в нём): ручная заливка игры для автора,
которому проще заполнить форму, чем диктовать уровень модели. Тот же сервер,
тот же клиент движка, та же организаторская сессия. Формы игры и задания
рисуются по той же JSON Schema, которую принимают инструменты агента
`admin_create_game` и `admin_create_level`, поэтому поле, добавленное агенту,
появляется и в форме: тест не даст им разойтись. В боковой панели — игры
организатора и задания выбранной игры с добавлением, удалением и
перестановкой; в главной области — поля по группам, таблицы основных,
бонусных, ложных и спойлерных кодов со вставкой списком, локальная проверка
перед записью, выгрузка и заливка сценария целиком и кнопка «Спросить агента»,
открывающая постоянный чат общего черновика игры.

Лист кодов вставляется не только из буфера: «Из файла» читает `.txt`, `.csv`
или `.tsv` — windows-1251 распознаётся сама, — и кладёт прочитанное в то же
поле вставки, чтобы автор увидел разбор до записи. Непрочитанное разбор
называет вслух, а не подставляет вместо него ноль или пустую сложность.
Поля автоматически сохраняются в локальный черновик. Переключение в чат
передаёт агенту актуальные несохранённые правки, а при возврате в редактор
виден результат его работы. Повторный переход открывает тот же чат.
Новые задания и черновик восстанавливаются после перезагрузки страницы.
Разные поля объединяются автоматически; при одновременной правке одного поля
интерфейс предлагает выбрать версию. В движок поля записываются только кнопкой
«Сохранить в игру» для открытой игры или задания. В привязанном чате агент
использует `draft_*`; инструменты изменения движка ему не выдаются.

Задание, подсказки и легенда — то, что движок показывает игрокам как HTML, —
правятся с панелью форматирования и переключателем на исходную разметку;
картинка кнопкой загружается в файлы самой игры, а относительные ссылки движка
разрешаются только для показа, так что на движок уходит ровно то, что в поле.

Организатором входят в самом редакторе: логин и пароль проверяются на админке и
сохраняются в тот же файл сессии, где их оставил бы `-admin-login`. Этот вход —
общий: после него в чате становятся доступны инструменты `admin_*`, потому что
набор инструментов собирается заново на каждый ход из того же клиента. Ни один
из двух видов не главный. Игровая сессия редактору не нужна — запуск
`dzzzr -admin-login … -admin-password … web` открывает только организаторскую
часть и сразу в редакторе. Адрес страницы (`#/editor/game/4242/level/901`)
называет открытую игру и задание, так что перезагрузка и «назад» возвращают
туда же, а без адреса страница открывает вид, в котором работали последним.
Подробно — в [docs/webui.md](docs/webui.md).

На Windows запуск `dzzzr.exe` без команды из Проводника открывает браузер
с редактором организатора. Из cmd.exe, PowerShell и других терминалов запуск
остаётся консольным: без команды выводится справка. Чтобы открыть редактор
из терминала явно, используйте `dzzzr editor`.

Способ запуска определяется по родительскому процессу; для неизвестных
запускателей используется число процессов в консоли. Если автоматически
запущенный сервер завершился с ошибкой, окно удерживается до нажатия Enter.
Любое непустое значение `DZZZR_NO_AUTO_WEB` выключает автозапуск —
...
| [![Редактор задания](https://skrashevich.github.io/dzzzr-cli/screenshots/editor.png)](https://skrashevich.github.io/dzzzr-cli/screenshots/editor.png) | [![Редактор, тёмная тема](https://skrashevich.github.io/dzzzr-cli/screenshots/editor-dark.png)](https://skrashevich.github.io/dzzzr-cli/screenshots/editor-dark.png) |
| Задания игры и форма уровня по группам полей | Тёмная тема |
| [![Вставка кодов списком](https://skrashevich.github.io/dzzzr-cli/screenshots/editor-bulk.png)](https://skrashevich.github.io/dzzzr-cli/screenshots/editor-bulk.png) | |
| Лист кодов вставляется одним блоком | |
...
| [docs/webui.md](docs/webui.md) | `dzzzr web`: редактор игры в браузере, его HTTP-поверхность и общая с агентом сессия |
