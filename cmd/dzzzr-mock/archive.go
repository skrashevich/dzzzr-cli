...
func (s *server) handleArchive(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(&b, `<!-- CONTENT --><h1>Архив игр</h1><div class=Date>%s</div><h1>%s</h1>`, g.Start.Format("02.01.2006"), html.EscapeString(g.GameName))
	switch q.Get("what") {
	case "stat":
		g.archiveStat(&b)
	case "comment":
...
func (g *gameState) archiveDescription(b *strings.Builder, allowed bool) {
	fmt.Fprintf(b, `<h2>Описание</h2><div class=grayBox><h2>Авторы</h2>%s</div><div class=grayBox><h2>Легенда</h2>%s</div>`, html.EscapeString(g.Authors), g.Legend)
	if !allowed {
	...
	for _, l := range g.Levels {
		fmt.Fprintf(b, `<h2>Задание: %s<br><span id=Code>Код: `, html.EscapeString(l.Title))
		for _, code := range l.Codes {
			fmt.Fprintf(b, "%s &nbsp;", html.EscapeString(code))
		}
		fmt.Fprintf(b, `</span></h2><div class=boxOrang id=left>%s<strong>Примечания к заданию</strong>: <p>%s</p><strong>Код сложности:</strong> %s<h3>Подсказка 1</h3>%s<h3>Подсказка 2</h3>%s</div>`, l.Question, html.EscapeString(l.LocationComment), html.EscapeString(strings.Join(l.Dangers, ",")), l.Clue1, l.Clue2)
		for _, sp := range l.Spoilers {
			fmt.Fprintf(b, `<div class=grayBox><h2>Спойлер %d</h2>%s<br>Код спойлера %d: %s</div>`, sp.Order, sp.Text, sp.Order, html.EscapeString(sp.Code))
		}
	}
}
...
func (g *gameState) archiveStat(b *strings.Builder) {
	...
	for _, l := range g.Levels {
		fmt.Fprintf(b, "<th>%s</th>", html.EscapeString(l.Title))
	}
	b.WriteString(`<!--levend--><th>Штраф</th><th>Бонусы</th><th>Чистое время</th><th>Общее время</th><th>Место</th></tr>`)
	fmt.Fprintf(b, `<tr><td><a href="?section=teams&teamID=%d">%s</a></td><!--lev-->`, g.TeamID, html.EscapeString(g.TeamName))
	...
	for _, l := range g.Levels {
		...
		fmt.Fprintf(b, "<td>%s</td>", value)
	}
	place := "—"
	if g.Finished {
		place = "1 / 1"
	}
	fmt.Fprintf(b, `<!--levend--><td>%d</td><td>%d</td><td>%s</td><td>%s</td><td>%s</td></tr></table>`, g.PenaltyMin, g.BonusMin, formatClock(total), formatClock(total+time.Duration(g.PenaltyMin-g.BonusMin)*time.Minute), place)
}
