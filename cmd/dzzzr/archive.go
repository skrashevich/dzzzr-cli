...
func cmdGameDesc(ctx context.Context, cfg *config, c *dzzzr.Client, args []string) error {
	...
	for i, l := range d.Levels {
		fmt.Fprintf(w, "\n\nЗадание %d: %s\n", i+1, dash(l.Title))
		if len(l.Codes) > 0 {
			fmt.Fprintf(w, "  Коды: %s\n", strings.Join(l.Codes, ", "))
		}
		if l.Difficulty != "" {
			fmt.Fprintf(w, "  Код сложности: %s\n", l.Difficulty)
		}
		if l.Task != "" {
			fmt.Fprintf(w, "\n%s\n", indent(l.Task, "  "))
		}
...
func printMediaRefs(w io.Writer, title string, refs []dzzzr.MediaRef) {
	...
	for _, r := range refs {
		switch {
		case r.Inline:
			fmt.Fprintf(w, "    картинка: встроена (%s, %s)\n", r.MediaType, humanBytes(r.Bytes))
...
func cmdGameLog(ctx context.Context, cfg *config, c *dzzzr.Client, args []string) error {
	...
	if len(log.Entries) == 0 {
		fmt.Fprintln(cfg.stdout, "Лог пуст.")
		return nil
	}
	...
	tw := newTable(cfg.stdout)
	fmt.Fprintln(tw, strings.Join(log.Columns, "\t"))
