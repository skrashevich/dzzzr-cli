...
func TestWebLLMSettingsGetReportsTheWholePanel(t *testing.T) {
	...
	if payload.SettingsPath != mustLLMSettingsFile(t) {
		t.Errorf("settings_path = %q, want %q", payload.SettingsPath, mustLLMSettingsFile(t))
	}
	...
	if payload.Codex.SignedIn || payload.Codex.Path != mustCodexAuthFile(t) {
...
func TestWebLLMSettingsPutRejectsBadInput(t *testing.T) {
	for _, tc := range []struct {
		...
		{"unknown auth method", `{"auth_method":"gigachat"}`, "неизвестный способ подключения"},
		...
		{"base url without a scheme", `{"base_url":"не-url"}`, "адрес API"},
		{"base url with no host", `{"base_url":"https://"}`, "нет хоста"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			if _, err := os.Stat(mustLLMSettingsFile(t)); !os.IsNotExist(err) {
...
// The DZZZR_-prefixed variable is the one the panel has to name: it is what
// the user set, and it wins over the generic LLM_* spelling.
func TestWebLLMSettingsNamesTheDZZZRVariableThatWins(t *testing.T) {
	...
	status, payload, raw := llmSettingsRequest(t, srv, http.MethodPut, `{"auth_method":"apikey","api_key":"sk-panel-key-9876","model":"ui/model"}`)
	if status != http.StatusOK {
		t.Fatalf("PUT status = %d, body %s", status, raw)
	}
	if strings.Contains(raw, "sk-panel-key-9876") {
		t.Fatalf("the response discloses the key: %s", raw)
	}
	var found bool
	for _, o := range payload.EnvOverrides {
		if o.Field == "model" {
			found = o.EnvVar == "DZZZR_LLM_MODEL" && o.ShadowsStored
		}
	}
	if !found {
		t.Fatalf("env_overrides = %+v, want model shadowed by DZZZR_LLM_MODEL", payload.EnvOverrides)
	}
	if stored, _ := loadLLMSettings(); stored.Model != "ui/model" {
		t.Fatalf("stored = %+v, want the panel's model kept", stored)
	}
	if payload.Agent.Model != "env/model" || payload.Agent.AuthMethod != authMethodAPIKey {
		t.Fatalf("agent = %+v", payload.Agent)
	}
}

// A user already signed in with Codex CLI is told so, rather than asked to
// sign in a second time.
func TestWebLLMSettingsReportsTheCodexCLISignIn(t *testing.T) {
	srv := newLLMSettingsTestServer(t)
	useCodexCLIFixture(t)
	_, payload, raw := llmSettingsRequest(t, srv, http.MethodGet, "")
	if !payload.Codex.CLISignIn || payload.Codex.SignedIn {
		t.Fatalf("codex = %+v (%s)", payload.Codex, raw)
	}
}

// Saving ChatGPT as the transport with only Codex CLI signed in runs the agent.
func TestWebLLMSettingsCodexOnTheCLISignIn(t *testing.T) {
	srv := newLLMSettingsTestServer(t)
	useCodexCLIFixture(t)
	status, payload, raw := llmSettingsRequest(t, srv, http.MethodPut, `{"auth_method":"codex"}`)
	if status != http.StatusOK || payload.Agent.Error != "" || payload.Agent.AuthMethod != authMethodCodex {
		t.Fatalf("status %d agent %+v (%s)", status, payload.Agent, raw)
	}
}
