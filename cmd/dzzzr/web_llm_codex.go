...
// stdout and blocks on stdin, so the web server cannot use it: it drives the same OAuth
...
// browser flow, and it is what releases the callback port when a user
...
func listenOnPort(port int) (net.Listener, int, error) {
	...
	if !ok {
		_ = l.Close()
		return nil, 0, fmt.Errorf("неожиданный тип адреса %T", l.Addr())
	}
...
// start opens a sign-in. With a listener the user only has to complete the
...
func (m *codexLoginManager) start(noLocalServer bool) (*codexLoginFlow, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("не удалось подготовить PKCE: %w", err)
	}
	state, err := auth.GenerateState()
	if err != nil {
		return nil, fmt.Errorf("не удалось подготовить state: %w", err)
	}
	id, err := newFlowID()
	if err != nil {
		return nil, err
	...
	if !noLocalServer {
		...
		if err != nil {
			return nil, fmt.Errorf(
				"для входа в ChatGPT нужен порт %d, а он занят (%w). "+
					"Закройте то, что его держит (другой вход dzzzr или Codex CLI), или вставьте адрес возврата вручную",
				cfg.Port, err)
		}
	...
	flow.timer.Store(time.AfterFunc(flow.ttl, func() {
		...
		flow.finish("", fmt.Errorf("вход в ChatGPT не завершён за %s", flow.ttl))
	}))
...
func (m *codexLoginManager) callbackHandler(flow *codexLoginFlow) http.Handler {
	...
	mux.HandleFunc("/auth/callback", func(w http.ResponseWriter, r *http.Request) {
		...
		if q.Get("state") != flow.state {
			flow.finish("", errors.New("адрес возврата содержит чужой state"))
		...
		if code == "" {
			...
			flow.finish("", fmt.Errorf("в адресе возврата нет кода: %s", msg))
		...
		fmt.Fprint(w, "<html><body><h2>Вход выполнен</h2><p>Можно закрыть эту вкладку и вернуться в dzzzr.</p></body></html>")
	})
	return mux
}
...
func (m *codexLoginManager) submitCode(id, input string) (*codexLoginFlow, error) {
	flow, ok := m.get(id)
	if !ok {
		return nil, errors.New("такого входа нет")
	}
...
func codeFromPastedInput(input, state string) (string, error) {
	...
	if input == "" {
		return "", errors.New("вставьте адрес возврата или код")
	}
	...
	if err != nil {
		return "", fmt.Errorf("это не адрес возврата: %w", err)
	}
	q := u.Query()
	if got := q.Get("state"); got != "" && got != state {
		return "", errors.New("вставленный адрес относится к другому входу")
	}
	code := strings.TrimSpace(q.Get("code"))
	if code == "" {
		if msg := strings.TrimSpace(q.Get("error")); msg != "" {
			return "", fmt.Errorf("вход отклонён: %s", msg)
		}
		return "", errors.New("во вставленном адресе нет кода авторизации")
	}
	return code, nil
}
...
func newFlowID() (string, error) {
	...
	if _, err := rand.Read(b[:]); err != nil {
		return "", fmt.Errorf("не удалось создать идентификатор входа: %w", err)
	}
...
type codexLoginRequest struct {
	// NoBrowser skips the local callback listener, for a user whose browser
...
func (h *webHub) httpCodexLoginStart(w http.ResponseWriter, r *http.Request) {
	...
	if r.ContentLength > 0 && !webReadJSON(w, r, &req) {
		return
	}
	flow, err := h.codexFlows().start(req.NoBrowser)
	if err != nil {
		webError(w, http.StatusConflict, "%s", err.Error())
		return
	}
	webWriteJSON(w, http.StatusCreated, flow.snapshot())
}

func (h *webHub) httpCodexLoginStatus(w http.ResponseWriter, r *http.Request) {
	flow, ok := h.codexFlows().get(r.PathValue("id"))
	if !ok {
		webError(w, http.StatusNotFound, "такого входа нет")
		return
	}
	webWriteJSON(w, http.StatusOK, flow.snapshot())
}
...
func (h *webHub) httpCodexLoginCode(w http.ResponseWriter, r *http.Request) {
	...
	if flow == nil {
		webError(w, http.StatusNotFound, "%s", err.Error())
		return
	}
	if err != nil {
...
func (h *webHub) httpCodexLoginCancel(w http.ResponseWriter, r *http.Request) {
	...
	flows.forget(id)
	webWriteJSON(w, http.StatusOK, map[string]string{"status": "cancelled"})
}
