...
// a developer machine may well have the Codex CLI or another dzzzr holding it.
...
func TestWebCodexStatusAndLogout(t *testing.T) {
	...
	if payload["path"] != mustCodexAuthFile(t) {
		t.Errorf("path = %v, want %q", payload["path"], mustCodexAuthFile(t))
	}
...
func TestWebCodexLoginExpiresAndReleasesThePort(t *testing.T) {
	...
	if msg, _ := payload["error"].(string); !strings.Contains(msg, "не завершён за") {
