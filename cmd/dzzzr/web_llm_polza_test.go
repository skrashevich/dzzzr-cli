...
func TestPolzaPKCESingleUsePersistenceAndSecrets(t *testing.T) {
	...
	q := auth.Query()
	if q.Get("response_type") != "code" || q.Get("code_challenge_method") != "S256" || q.Get("state") != f.state || q.Get("app_name") != "dzzzr" {
...
// Polza serves many upstreams; the agent asks it for the fastest one that
// handles tool calls, exactly as encli does.
func TestResolveLLMConfigSendsPolzaProviderPreferences(t *testing.T) {
	isolateLLMEnv(t)
	writeTestLLMSettings(t, llmSettings{AuthMethod: authMethodAPIKey, BaseURL: polzaBaseURL, APIKey: "k", Model: "openai/gpt-4.1"})
	...
	if cfg, _ := resolveLLMConfig(); cfg.ExtraBody != nil {
		t.Fatalf("OpenRouter must not get Polza's preferences: %#v", cfg.ExtraBody)
	}
}
